...
def get_figures(sim_result):
    ...
    fig_temp_dict["data"] = [go.Scatter(x=timestamps__hm, y=sim_result.temperature_trends[:, hn]) for hn in
                             range(5)]  # [4, 7, 5, 3, 2])
    ...
    fig_power_dict["data"] = [go.Scatter(x=timestamps__hm, y=sim_result.power_trend, line={'color': 'mediumseagreen'}),
                              go.Scatter(x=timestamps__hm, y=sim_result.avg_power_trend, line={'color': 'slateblue'})]
    ...
    figure_power.update_layout(template=template)
    figure_power.add_vrect(x0=8 * 3600 / step_size__sec, x1=9 * 3600 / step_size__sec,
                           fillcolor="tomato", opacity=0.2, layer="below", line_width=0,
                           )
    ...
    fig_cost_dict["data"] = [go.Scatter(x=timestamps__hm, y=sim_result.cost_trend, line={'color': 'mediumseagreen'})]
